...
public class ProcessDownloadedImageRunnableForGroup implements Runnable {
    final GroupChatData mDataTextChat;
    Context mContext;
    private Handler uiHandler;

    public ProcessDownloadedImageRunnableForGroup(Context mContext, final GroupChatData mDataTextChat, Handler uiHandler) {
...
//            if (mDataTextChat.getIsMasked().equalsIgnoreCase("1")) {
            ...
            Bitmap data = BitmapFactory.decodeFile(mDataTextChat.grpcText, options);
            Bitmap fixRotationBitmap = MediaUtils.fixImageRotation(data, mDataTextChat.grpcText);
            if (fixRotationBitmap != null) {
                String[] filePathSplit = mDataTextChat.grpcText.split("/");
