...
public class PhotoEditorSDK implements MultiTouchListener.OnMultiTouchListener {
    ...
    public void addImage(Bitmap desiredImage) {
        ...
        View imageRootView = inflater.inflate(R.layout.photo_annotate_sdk_image_item_list, null);
    ...
    public void addText(String text, int colorCodeTextView, float x, float y) {
        LayoutInflater inflater = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        addTextRootView = inflater.inflate(R.layout.photo_annotate_sdk_text_item_list, null);
    ...
    public void addEmoji(String emojiName, Typeface emojiFont) {
        LayoutInflater inflater = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        View emojiRootView = inflater.inflate(R.layout.photo_annotate_sdk_text_item_list, null);
    ...
    public String saveImage(String folderName, String imageName,Context context) {
        String selectedOutputPath = "";
        try {

            File file = MediaUtils.getOutputMediaFileInPublicDirectory(ConstantUtil.SKETCH_TYPE, context, imageName+"");
            selectedOutputPath = file.getPath();
            try {
            ...
            Log.e("Exception","sketch save exception");
        }

       /* if (isSDCARDMounted()) {
           // File mediaStorageDir = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_PICTURES), folderName);
            File mediaStorageDir = new File(Environment.getExternalStorageDirectory().getPath()+ "/Vhortext");
            // Create a storage directory if it does not exist
            if (!mediaStorageDir.exists()) {
                if (!mediaStorageDir.mkdirs()) {
                    Log.d("Vhortext", "Failed to create directory");
                }
            }
            // Create a media file name
            selectedOutputPath = mediaStorageDir.getPath() + File.separator + imageName;
            Log.d("Vhortext", "selected camera path " + selectedOutputPath);
            ...
            // File mediaStorageDir = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_PICTURES), folderName);
            File mediaStorageDir = new File(context.getFilesDir().getPath()+ "/Vhortext");
            // Create a storage directory if it does not exist
            if (!mediaStorageDir.exists()) {
                if (!mediaStorageDir.mkdirs()) {
                    Log.d("Vhortext", "Failed to create directory");
    ...
    @Override
    public void onEditTextClickListener(String text, int colorCode, float x, float y) {
        Log.e("onEditTextClickListener",text);
        if (addTextRootView != null) {
            //parentView.removeView(addTextRootView);
            //addedViews.remove(addTextRootView);
            for(int i=0;i<addedViews.size();i++){
                if(addedViews.get(i).getX()==x && addedViews.get(i).getY()==y){
                    Log.e("remove view", String.valueOf(addedViews.size())+" -- "+ String.valueOf(i));
                    //parentView.removeView(addTextRootView);
                    parentView.removeView(addedViews.get(i));
                    addedViews.remove(i);
                }
            ...
            System.out.println("onEditTextClickListener Text : "+text+" ID : "
                    +addTextRootView.getId()+" X : "+addTextRootView.getX()
                    +" Y : "+addTextRootView.getY()/*+" Z : "+addTextRootView.getZ()*/);
        }
    }
